/************* Problem Statement *************
 ...
 If n is even, divide by 2. If n is odd, multiply by 3 and add 1. Repeat this process with the new value of n,
 ...
 Output
 For each pair of input integers i and j, output i, j in the same order in which they appeared in the input and
 then the maximum cycle length for integers between and including i and j. These three numbers should be separated
 by one space, with all three numbers on one line and with one line of output for each line of input.
...
public class The3nPlusOne_110101 {

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        while (sc.hasNext()) {
            int low = sc.nextInt(), hi = sc.nextInt();
            System.out.println(low + " " + hi + " " + getMaxCount(low, hi));
        }
    }

    public static long getMaxCount(int low, int high) {
        // Given input may contain higher value first and then lower value. In that case, swap them.
        if (low > high) {
            int tmp = low;
            low = high;
            high = tmp;
        ...
        for (int i = low; i <= high; i++) {
            // 3n+1 value may overflow the integer value, so use long.
            long j = i, count = 1;
            while (j != 1) {
                if (j % 2 == 0) {
                    j = j / 2;
